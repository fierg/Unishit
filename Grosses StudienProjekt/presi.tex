...
\end{itemize}	
		...
		\frame{\frametitle{Einführung}
	todo - motivation invertierter index
	}	
...
\begin{columns}
	...
	\end{itemize}
\end{column}
 \pause \begin{column}{0.47\textwidth}
\begin{algorithm}[H]
...
\item Zeitkompplexität:	$\Theta ( T )$, kein Sortieren von TermID-DocID Paaren, alle Operationen linear
\end{itemize}
}
...
\end{column}
\begin{column}{0.47\textwidth}
\only<2>{\includegraphics[scale=0.4]{pdf/mapreduce.pdf}}
\end{column}
\end{columns}
}
...
\begin{itemize}
\item Webseiten geändert, gelöscht oder neue hinzugefügt..
\end{itemize}
\pause \item Indexerstellung über eine solche Sammlung ebenfalls dynamsich
\end{itemize}
}
...
\item zugriffsbeschränkte Indizes
\item \enquote{in situ}-Indexerstellung
\end{itemize}
}
